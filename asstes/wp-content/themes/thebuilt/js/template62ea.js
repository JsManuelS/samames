...
$(document).ready(function() {
	...
	*/	
	...
	// Menu Widget
	$('.sidebar.main-sidebar .widget.widget_nav_menu a').on('click', function(e){
...
function thebuilt_facebookShare(){
	window.open( 'https://www.facebook.com/people/Samames-Construction/61558384208723//sharer/sharer.php?u='+window.location, "facebookWindow", "height=380,width=660,resizable=0,toolbar=0,menubar=0,status=0,location=0,scrollbars=0" ) 
	return false;
}
...
// Función para cambiar el color de los enlaces al hacer scroll
function cambiarColorEnlaces() {
	...
	// Cambiar el color de los enlaces dependiendo de la posición de desplazamiento
	links.forEach(function(link) {
		if (scrollPosition > 50) {
			link.style.color = 'black'; // Cambiar a color negro
		} else {
			link.style.color = 'white'; // Mantener color blanco
		}
	});
}

// Llamar a la función al cargar la página para establecer el color inicial
window.addEventListener('load', cambiarColorEnlaces);

// Llamar a la función al hacer scroll para cambiar el color de los enlaces
window.addEventListener('scroll', cambiarColorEnlaces);

// movil para cambio //
// Función para verificar si es un dispositivo móvil y aplicar la clase correspondiente al contenedor del logo
function mostrarLogoBlancoEnMovil() {
    var logoContainer = document.querySelector('.header-left.logo');

    // Verificar si es un dispositivo móvil basado en el ancho de la pantalla
    if (window.innerWidth <= 768) { // Cambia este valor según lo necesites
        logoContainer.classList.add('mostrar-light-logo'); // Aplicar clase para mostrar el logo blanco
    } else {
        logoContainer.classList.remove('mostrar-light-logo'); // Quitar clase para ocultar el logo blanco
    }
}

// Llamar a la función al cargar la página y al cambiar el tamaño de la pantalla
window.addEventListener('load', mostrarLogoBlancoEnMovil);
window.addEventListener('resize', mostrarLogoBlancoEnMovil);


///para laborar despegable 

document.addEventListener('DOMContentLoaded', function() {
    var laborarToggle = document.getElementById('laborar-toggle');
    var dropdownMenu = document.querySelector('.dropdown-menu');
	

    laborarToggle.addEventListener('click', function(e) {
        e.preventDefault();
...
//INICIO//
document.addEventListener('DOMContentLoaded', function() {
	var inicioToggle = document.querySelector('.inicio-toggle'); // Cambiar a tu ID específico
	var inicioDropdown = document.querySelector('.p-inicio'); // Cambiar a tu selector específico

	inicioToggle.addEventListener('click', function(e) {
		e.preventDefault();
		inicioDropdown.style.display = (inicioDropdown.style.display === 'block') ? 'none' : 'block';
	});
});
...
//SERVICIO
document.addEventListener('DOMContentLoaded', function() {
	var serviciosToggle = document.querySelector('.servicios-toggle'); // Cambiar a tu ID específico
	var serviciosDropdown = document.querySelector('.p-servicios'); // Cambiar a tu selector específico

	serviciosToggle.addEventListener('click', function(e) {
		e.preventDefault();
		serviciosDropdown.style.display = (serviciosDropdown.style.display === 'block') ? 'none' : 'block';
	});
});
...
//PAGINAS
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var paginasToggle = document.querySelector('.paginas-toggle');
        var paginasDropdown = document.querySelector('.p-paginas'); // Cambiar a tu selector específico

        paginasToggle.addEventListener('click', function(e) {
            e.preventDefault();
            paginasDropdown.style.display = (paginasDropdown.style.display === 'block') ? 'none' : 'block';
        });
    });
...
//CONTACTO
document.addEventListener('DOMContentLoaded', function() {
	var contactoToggle = document.querySelector('.contacto-toggle');
	var contactoDropdown = document.querySelector('.p-contacto');

	contactoToggle.addEventListener('click', function(e) {
		e.preventDefault();
		contactoDropdown.style.display = (contactoDropdown.style.display === 'block') ? 'none' : 'block';
	});
});